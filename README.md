...
- 「音乐」页面通过**网易云音乐官方外链播放器**（iframe）分享流媒体歌曲，嵌入配置在 `lib/music.ts` 的 `neteaseEmbeds` 数组（`type: 'song' | 'playlist'` + `id` + 标题说明）。
- 获取嵌入 ID：网易云音乐网页版打开歌曲/歌单分享页 →「生成外链播放器」→ 复制链接中的 `id`；单曲 `type` 填 `'song'`，歌单填 `'playlist'`。
- 版权边界：部分歌曲因版权禁止外链（页面会显示平台提示），VIP 歌曲对未登录访客可能只有试听片段；该页面播放行为由网易云平台提供，与上方自制播放器相互独立。
- 已在 `proxy.ts` 的 CSP 中为该页面放行 `frame-src https://music.163.com`。
