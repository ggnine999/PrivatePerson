...
export async function POST(request: Request) {
  ...
  const score = typeof input.score === 'number' ? Math.floor(input.score) : -1;
  const accuracy = typeof input.accuracy === 'number' ? Math.floor(input.accuracy) : -1;
  const maxCombo = typeof input.maxCombo === 'number' ? Math.floor(input.maxCombo) : -1;
  if (
    score < 0 ||
    score > MAX_SCORE ||
    accuracy < 0 ||
